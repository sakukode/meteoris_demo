StudentsController = MeteorisController.extend({
    ...
    subscriptions: function() {
        ...
        this.subscription = this.subs.subscribe('students', this.getCriteria(), sort);
    },
...
{birthDate: {$regex: search, $options: 'i'}},
    ...
    index: function() {
        var sort = MeteorisGridView.getSorting();
        sort.limit = this.limit();
        var models = Students.find(this.getCriteria(), sort);

        return this.render('studentsIndex', {
            data: {
...
birthDate: t.find('#birthDate').value? new Date(t.find('#birthDate').value):null,
    ...
    insert: function(t) {
        if (this._post) {
            ...
            Students.insert(doc, function(err, _id) {
                if (err) {
                    MeteorisFlash.set('danger', err.message);
                    throw new Meteor.Error(err);
                }
                MeteorisFlash.set('success', "Success Inserting Students");
                Router.go('studentsView', {_id: _id});
            });
        }
    ...
    update: function(t) {
        ...
        if (this._post) {
            ...
            Students.update(_id, {$set: doc}, function(err) {
                if (err) {
                    MeteorisFlash.set('danger', err.message);
                    throw new Meteor.Error(err);
                }
                MeteorisFlash.set('success', "Success Updating Students");
            });
            Router.go('studentsView', {_id: _id});
        }
        return this.render('studentsUpdate', {
            data: {
                model: model,
            }
        });
    ...
    remove: function(_id) {
        Students.remove(_id, function(err) {
            if (err) {
                MeteorisFlash.set('danger', err.message);
                throw new Meteor.Error(err);
            }
            MeteorisFlash.set('success', "Success Removing Students");
        });
    },
